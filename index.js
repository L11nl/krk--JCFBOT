...
function getWorkspaceStatusEmoji(used) {
...
async function getWorkspaceUsedSeats(wsId) {
    const stats = await getWorkspaceStats(wsId);
    return Number(stats.member_count || 0) + Number(stats.invite_count || 0);
}
...
async function getWorkspaceEmailsFromDb(wsId) {
    const rows = await dbAll('SELECT email FROM allowed_emails WHERE ws_id = ? ORDER BY id ASC', [wsId]);
...
async function notifyWorkspaceLoggedOut(ws) {
    try {
...
اختر الخدمة المناسبة:`, {
    ...
    if (accessRow && accessRow.role === 'trader') {
        ...
        const lines = [
            ...
            'اختر المساحة التي تريد إدارتها.'
        ];
        if (count >= 7) {
        ...
        const traderKeyboard = [];
        for (const ws of workspaces) traderKeyboard.push([{ text: await getWorkspaceButtonLabel(ws), callback_data: `ws_open_${ws.id}` }]);
        traderKeyboard.push([{ text: 'ملاحظة', callback_data: 'trader_note' }]);
        if (count >= 7) traderKeyboard.push([{ text: '💳 تسديد الديون', callback_data: 'trader_show_payment' }]);
        return await bot.sendMessage(chatId, lines.join('\n'), { reply_markup: { inline_keyboard: traderKeyboard } });
    }

    const text = headerText || 'مرحبا ببوت ادارة مساحات العمل';
    const workspaces = await getVisibleWorkspaces(chatId);
    const inline_keyboard = [];
    for (const ws of workspaces) inline_keyboard.push([{ text: await getWorkspaceButtonLabel(ws), callback_data: `ws_open_${ws.id}` }]);

    if (!(await isTrader(chatId))) {
        inline_keyboard.push([{ text: 'اضافة مساحة (تلقائي سريع ⚡)', callback_data: 'add_workspace_auto' }]);
        inline_keyboard.push([{ text: 'تسجيل دخول بالايميل والباسورد', callback_data: 'add_workspace_step_login' }]);
...
async function completeLoginFlow(page, state, chatId) {
    ...
    const code6 = state.direct2faCode ? String(state.direct2faCode).replace(/\D/g, '').slice(0, 6) : await fetch2FACode(page.context(), state.url2fa);
    if (!code6 || code6.length < 6) throw new Error('كود 2FA غير صالح');
    await page.bringToFront();
    await sendAdminStep(chatId, page, `تم تجهيز كود 2FA: ${code6}`);
...
async function clickFirstVisible(page, selectors = [], timeout = 2000) {
    for (const sel of selectors) {
        const loc = page.locator(sel).first();
...
async function clickByTextRegex(page, regex, selectors = 'button, [role="button"], a, div[role="button"]') {
    const rxSource = regex instanceof RegExp ? regex.source : String(regex);
    const coords = await page.evaluate((source, flags, selectors) => {
        const rx = new RegExp(source, flags || 'i');
        const els = Array.from(document.querySelectorAll(selectors)).filter(el => {
            const rect = el.getBoundingClientRect();
            const txt = (el.innerText || el.textContent || '').trim();
            return rect.width > 0 && rect.height > 0 && txt && rx.test(txt);
        });
        ...
        return { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
    }, rxSource, regex.flags || 'i', selectors).catch(() => null);
    if (!coords) return false;
    await page.mouse.click(coords.x, coords.y).catch(() => {});
    return true;
}

async function findFirstVisibleLocator(page, selectors = [], timeout = 2000) {
    for (const sel of selectors) {
        const loc = page.locator(sel).first();
...
if (!(await hasBotAccess(chatId))) return bot.sendMessage(chatId, '❌ البوت غير مفعل لديك بعد.');
    ...
    if (data === 'add_workspace_step_login') {
        if (await isTrader(chatId)) return bot.sendMessage(chatId, '❌ هذا الخيار غير متاح للتاجر.');
        state.mode = 'step_login';
        state.email = '';
        state.password = '';
        state.url2fa = '';
        state.direct2faCode = '';
        state.step = 'step_login_email';
        return bot.sendMessage(chatId, 'ارسل إيميل المساحة:');
    }
...
اختر طريقة الدفع الآن لإكمال الطلب خلال ساعة واحدة.`);
    ...
    if (state.step === 'step_login_email') {
        state.email = normalizeEmail(text);
        state.step = 'step_login_password';
        return bot.sendMessage(chatId, 'ارسل الباسورد الآن:');
    }

    if (state.step === 'step_login_password') {
        state.password = String(text || '').trim();
        state.step = 'step_login_code';
        return bot.sendMessage(chatId, 'ارسل الآن كود 2FA أو رابط 2FA. يمكنك ارسال 123456 أو رابط يبدأ بـ https://');
    }

    if (state.step === 'step_login_code') {
        ...
        if (/^https?:\/\//i.test(raw)) {
            ...
            if (code.length < 6) return bot.sendMessage(chatId, '⚠️ ارسل كود صحيح من 6 أرقام أو رابط 2FA.');
            state.direct2faCode = code;
            state.url2fa = '';
        }
        state.mode = 'auto';
        ...
        try {
            ...
            let statusMsg = await bot.sendMessage(chatId, `⏳ جاري تسجيل الدخول للمساحة...`);
            ...
            try {
                ...
                await syncWorkspaceStatsFromPage(insertedId, members, invites);
                await bot.deleteMessage(chatId, statusMsg.message_id).catch(() => {});
                state.currentWsId = insertedId; state.currentTab = 'members'; state.step = null;
                await bot.sendMessage(chatId, `✅ تمت إضافة المساحة بنجاح: ${wsName}`);
                await pushUpdatedWorkspaceMenu(chatId, '✅ تمت إضافة المساحة الجديدة.');
                await state.page.close().catch(() => {}); state.page = null; state.context = null; state.direct2faCode = '';
            } catch (autoError) {
                await bot.deleteMessage(chatId, statusMsg.message_id).catch(() => {});
                bot.sendMessage(chatId, `⚠️ فشل الدخول التلقائي: ${autoError.message}`);
                state.context = null; state.step = null;
            }
        } catch (error) {
            state.step = null;
            return bot.sendMessage(chatId, `❌ خطأ فادح: ${error.message}`);
        }
        return;
    }
...
${text}`);
    ...
    if (state.step) {
        ...
        if (state.step === 'ws_awaiting_add_person') {
            if (!wsId) { state.step = null; return bot.sendMessage(chatId, '❌ اختر المساحة أولاً.'); }
            state.step = 'processing'; let statusMsg = await bot.sendMessage(chatId, '⏳ جاري إضافة الإيميل إلى المساحة المحددة...');
            try {
                ...
                const ws = await dbGet('SELECT * FROM workspaces WHERE id = ?', [wsId]);
                if (!ws) throw new Error('المساحة غير موجودة.');
                if (!fs.existsSync(ws.profile_dir)) throw new Error('ملف دخول المساحة غير موجود.');
                const stats = await getWorkspaceStats(ws.id);
                const used = Number(stats.member_count || 0) + Number(stats.invite_count || 0);
                if (used >= MEMBER_LIMIT) throw new Error(`المساحة ممتلئة (${used}/${MEMBER_LIMIT}).`);
...
ثم اضغط على ( ${ws.name} )`);
        ...
        if (state.step === 'ws_awaiting_change_name') {
            ...
            try {
                ...
                const nameInput = await findFirstVisibleLocator(page, [
                ...
                if (nameInput) {
                    ...
                    await page.keyboard.type(textInput, { delay: typeDelay(45) });
                    await sleep(500);
                    ...
                    ], 1500) || await clickByTextRegex(page, /save|update/i);
                    if (!saved) await page.keyboard.press('Enter').catch(() => {});
                    await sleep(1200);
                } else {
                    ...
                    await page.mouse.click(inputCoords.x, inputCoords.y, { clickCount: 3 }); await page.keyboard.press('Backspace'); await page.keyboard.type(textInput, { delay: typeDelay(45) }); await sleep(400); await page.keyboard.press('Enter').catch(() => {}); await sleep(1200);
                }
                await dbRun('UPDATE workspaces SET name = ? WHERE id = ?', [textInput, wsId]); await bot.deleteMessage(chatId, statusMsg.message_id).catch(() => {}); await bot.sendMessage(chatId, '✅ تم وضع الاسم الجديد وحفظه.'); await page.close();
