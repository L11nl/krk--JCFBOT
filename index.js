...
async function addEmailIntoAvailableWorkspace(actorChatId, email, opts = {}) {
    ...
    const list = await getCandidateWorkspacesForActor(actorChatIdNormalized, actorRole, opts.excludeWorkspaceId || null);
    ...
    for (const ws of list) {
        ...
        try {
            ...
            if (memberSet.has(norm)) { await page.close().catch(() => {}); continue; }
            const count = memberSet.size;
            if (count >= MEMBER_LIMIT) { await page.close().catch(() => {}); continue; }
            ...
            if (inviteSet.has(norm)) { await page.close().catch(() => {}); continue; }
            await page.goto('https://chatgpt.com/admin/members?tab=members', { waitUntil: 'domcontentloaded', timeout: 45000 });
            await sleep(900);
            ...
            return { ok: true, email: norm, workspaceId: ws.id, workspaceName: ws.name, remaining: Math.max(0, MEMBER_LIMIT - (count + 1)) };
        } catch (e) {
            await page.close().catch(() => {});
        }
    }
...
bot.on('photo', async (msg) => {
    ...
    if (!sessions[chatId]) sessions[chatId] = { step: null, currentTab: 'members' };
    const state = sessions[chatId];
    if (state.step !== 'trader_waiting_payment_photo') return;
    const row = await getUserAccess(chatId);
